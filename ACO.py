...
def Fitness(POP, num_iter):
      POP_F = []
      for particle in POP:
            for index2 in range(len(particle)):
                  if particle[index2] >= max_values[index2]:
                        particle[index2] = max_values[index2]
                  elif particle[index2] <= min_values[index2]:
                        particle[index2] = min_values[index2]
...
def Distance_norm(POP):
      ...
      for ant in POP:
            ...
            for other_ant in POP:
                  ...
                  individual_distances.append(np.linalg.norm((a - b), ord=1))
...
def Weigths(POP, POP_F):
      ...
      for index1 in range(len(ant_fit_sort)):
            ...
            weigth = (1/(q_ant*size_pop*(math.sqrt(2*math.pi)))) * math.exp((-(index1**2))/(2*(q_ant**2)*(size_pop**2)))
...
def Average(POP, selected_index, index2):
      ...
      for index1 in range(len(POP)):
            ...
            if index1 != selected_index:
                  distances.append(abs(POP[index1][index2] - selected[index2]))
...
def ACO():
      ...
      while num_iter < MFES:
            ...
            POP_all = POP + NEW_POP
            POP_all_F = POP_F + NEW_POP_F

            POP_pre = Selection_mode(POP_all, POP_all_F)

            POP = POP_pre[:size_pop]
      ...
      print(POP_F)
      best_in = min(POP_all_F) - global_min
      worst_in = max(POP_all_F) - global_min
      mean_in = np.mean(POP_all_F) - global_min
      median_in = np.median(POP_all_F)
      ...
      print("End in generation :", gen_id)
      print("End in Run nº: ", num_iter)
      print("Minimum result found in: ", POP_all[POP_all_F.index(min(POP_all_F))], " : ", best_in)
      return best_in, worst_in, mean_in, median_in, best_par, num_iter, success #maximum_apt, average_apt, minimum_apt,
...
def Output_Excel(number_runs):
      ...
      wb.save('CEC2005 Function4 - ACO_T2.xls') 
