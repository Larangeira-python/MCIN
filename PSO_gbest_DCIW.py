...
def Mean_pos(birds):
    ...
    for index2 in range(dim):
        ...
        for index1 in range(size_pop):
            average += birds[index1].position[index2]
        
        mean_pos.append(average/size_pop)
...
def Dispersion(birds, mean_pos, long_diag):
    sum_flock = 0

    for index1 in range(size_pop):
        sum_bird = 0
        for index2 in range(dim):
            sum_bird += (birds[index1].position[index2] - mean_pos[index2])**2

        sum_flock += sum_bird**0.5

    print("Overall distances: ", sum_flock)

    return (1/(size_pop*long_diag))*sum_flock
...
def PSO():
    birds = InitialPop()
    long_diag = Long_diag(dim)
    mean_pos = Mean_pos(birds)
    dispersion_before = Dispersion(birds, mean_pos, long_diag)
    ...
    while num_iter < MFES:
        ...
        dispersion_after = Dispersion(birds, mean_pos, long_diag)

        w = 1.4 * (1 - dispersion_after/dispersion_before)

        print("General inertia :", w)
        print("Dispersion :", dispersion_after)
